...
- Added CSS files to package `sideEffects` field to prevent CSS removal in some bundlers ([#659](https://github.com/incmix/ui-base/pull/659))
...
- Add standalone entrypoints for all components. For example:
  ```tsx
  import { AlertDialog } from '@incmix/ui-base/components/alert-dialog';
  import { Box } from '@incmix/ui-base/components/box';
  // We recommend using namespaced imports for compound components
  import * as CheckboxGroup from '@incmix/ui-base/components/checkbox-group';
  ```
- Fix visibility flash with closing dialogs ([#649](https://github.com/incmix/ui-base/pull/649))
- Fix regression that caused Segmented Control indicators to lose box shadows ([#650](https://github.com/incmix/ui-base/pull/657))
- Add `aria-hidden="true"` to duplicate nodes in `SegmentedControl.Item` to prevent double-reading by screen readers ([#651](https://github.com/incmix/ui-base/pull/651))
...
- Fix responsive styles for `gridArea` prop ([#569](https://github.com/incmix/ui-base/pull/569))
- Add `disabled` prop to `SegmentedControl.Root` ([#621](https://github.com/incmix/ui-base/pull/621))

## 3.1.4

- Export `ThemeContext` ([#582](https://github.com/incmix/ui-base/pull/582))
- Export `Popover.Anchor` ([#588](https://github.com/incmix/ui-base/pull/588))
...
- General
  - Package structure
    ...
    - **[Breaking]** Remove component prop definitions and internal helpers from the root `@incmix/ui-base` import entry point and export them from `@incmix/ui-base/props` and `@incmix/ui-base/helpers` to make it possible to build your own component library on top of Radix Themes using the same techniques.
  ...
  - Props
    ...
    - **[Breaking]** The `width` and `height` props don't map to space scale anymore. This is because in the vast majority of cases, width and height were not set to space scale, and with that, space scale as an IDE autocomplete suggestion felt odd/misleading.
      - **[Upgrade guide]** Find and replace your `width` and `height` prop usage with the corresponding [space scale](https://github.com/incmix/ui-base/blob/main/packages/radix-ui-themes/src/styles/tokens/space.css) steps, e.g. `width="9"` would need to be changed to `width="64px"` or `width="var(--space-9)"`, and so on for other steps.
...
- General:
  - **[Breaking]** Import styles from `@incmix/ui-base/styles.css` instead of `@radix-ui/react-themes/index.css`
- `ContextMenu`:
...
- General
  - **[Breaking]** Import styles from `@incmix/ui-base/index.css` instead of `@radix-ui/react-themes/dist/index.css`
